/* ==========================================================================
   ...
   Google Analytics 4 stays off until SITE.ga4 holds the site's measurement
   ID, "ga4": "G-..." (see "analytics" below).
...
(function () {
  ...
  /* Has this site had its SEO and AEO layer written into the HTML by the
     local-seo-aeo skill? Its marker comment sits in the head. Where it has,
     the file is the source of truth for everything a crawler reads. */
  var _baked = null;
  function baked() {
    if (_baked === null) {
      _baked = /seo-bake:start/.test(document.head ? document.head.innerHTML : "");
    }
    return _baked;
  }
  ...
  function fillTemplate() {
    ...
    var filled = 0;
    if (b.name) {
      /* A baked page's head was written for search engines and for the AI
         assistants, whose crawlers never run this script. Overwriting it here
         would leave the two disagreeing, so on a baked page the written-in
         title and description stand and only the visible text is filled. */
      if (!baked()) document.title = copy.metaTitle || (b.name + (b.town ? " | " + b.town : ""));
      filled += set(".navbar__brand", b.shortName || b.name);
      filled += set("[data-gk='name']", b.name);
    }
    if (copy.metaDescription && !baked()) {
    ...
    /* Contact links, so every "call us" on the page reaches the real business
       ...
       Only the number or address itself is rewritten, never the link's whole
       text. A button is a label span beside an icon span, a footer link is
       often an arrow followed by the number, and setting textContent on the
       link wiped both. The match is found in the link's text as a whole and
       written back into just the text nodes it covers, so every element stays
       and the words around it, such as "Call ", are kept. */
    var swapText = function (link, match, value) {
      if (!match) return;
      var walk = document.createTreeWalker(link, NodeFilter.SHOW_TEXT);
      var pieces = [], text = "", node;
      while ((node = walk.nextNode())) {
        pieces.push({ node: node, start: text.length });
        text += node.nodeValue;
      }
      var from = text.indexOf(match), to = from + match.length, placed = false;
      if (from < 0) return;
      pieces.forEach(function (piece) {
        var own = piece.node.nodeValue, start = piece.start;
        if (start + own.length <= from || start >= to) return;
        /* A number split over several nodes, one span per glyph for instance,
           gets the whole replacement in its first node and the rest emptied. */
        piece.node.nodeValue = own.slice(0, Math.max(from - start, 0)) +
          (placed ? "" : value) + own.slice(to - start);
        placed = true;
      });
    };
    /* A phone number the way templates print one: "(612) 555-0119",
       "0121 496 0166", "+44 7700 900318". Seven digits or more, so "24/7" or
       "Open 7 days" beside a phone icon stays as the designer wrote it. */
    var phoneIn = function (text) {
      var run = /(?:\+?\d{1,3}[\s.-]?)?(?:\(\d{1,5}\)[\s.-]?)?\d{2,}(?:[\s.-]\d{2,})*/g;
      var found;
      while ((found = run.exec(text))) {
        if (found[0].replace(/\D/g, "").length >= 7) return found[0];
      }
      return "";
    };
    ...
    if (b.phone) {
      ...
      Array.prototype.forEach.call(document.querySelectorAll('a[href^="tel:"]'), function (a) {
        ...
        swapText(a, phoneIn(a.textContent), b.phone);
        filled++;
      });
    }
    if (b.email) {
      Array.prototype.forEach.call(document.querySelectorAll('a[href^="mailto:"]'), function (a) {
        ...
        swapText(a, (a.textContent.match(/[\w.%+'-]+@[\w-]+(?:\.[\w-]+)+/) || [""])[0], b.email);
        filled++;
      });
    }
    return filled;
  ...
  function addSchema() {
    var b = SITE.business || {};
    if (!b.name) return;
    /* A baked page already carries a fuller graph than this one, and two
       LocalBusiness blocks on a page is worse than either alone. */
    if (baked() || document.querySelector('script[type="application/ld+json"]')) return;
      ...
      form.addEventListener("submit", function (event) {
        ...
        var done = function (ok) {
          form.innerHTML = '<p class="gk-form__done">' +
            ...
            "</p>";
          if (ok) track("generate_lead", { lead_source: "hero form" });
        };
  ...
  /* -------------------------------------------------------------------- chat
     Answers from a short scripted set by default, from the site's own AI
     backend once it is switched on (see "AI replies" below), and from
     SITE.chat.endpoint when one is wired. The point of it on a demo is that
     the owner sees an assistant sitting on their site at midnight, so it must
     never look broken: an unanswerable question hands over to the phone
     number rather than apologising in a loop. */
  function renderChat() {
    var c = SITE.chat;
    ...
    /* The visitor's first message on this page load, typed or a tapped
       question, whether the answer is scripted or AI. */
    ...
    /* ------------------------------------------------------------ AI replies
       api/chat.js, deployed beside the site, answers from the business's own
       knowledge file once a free Groq key is set on the Vercel project. Until
       then it reports configured: false and every answer stays scripted.

       Asked once, the first time the panel opens. The path is absolute, so a
       template previewed from a library subfolder gets a 404 and keeps its
       script. A site wired to its own SITE.chat.endpoint is left on that. */
    var AI_PATH = "/api/chat";
    var ai = { on: false, checked: null, queue: null, turns: [], id: "" };

    /* One id per browser tab, sent with every AI message so the owner's CRM
       keeps a visitor's conversation together, across a reload too. Where the
       browser blocks storage it lasts for this page load only. */
    var CHAT_ID = "gk_chat_id";
    function conversationId() {
      if (ai.id) return ai.id;
      var id = "";
      try { id = sessionStorage.getItem(CHAT_ID) || ""; } catch (e) { /* storage blocked */ }
      if (!/^[a-z0-9]{20}$/.test(id)) {
        ...
        id = "";
        for (var j = 0; j < bytes.length; j++) id += letters.charAt(bytes[j] % letters.length);
        try { sessionStorage.setItem(CHAT_ID, id); } catch (e) { /* this page load only */ }
      }
      ai.id = id;
      return id;
    }

    /* Resolves with the status and the parsed JSON body. A network error, a
       body that is not JSON (a static host's 404 page) or no answer within ms
       all reject, and every caller treats a rejection as "use the script". */
    function aiFetch(options, ms) {
    ...
    function checkAi() {
      if (!ai.checked) {
        ai.checked = (c.endpoint || !/^https?:$/.test(location.protocol))
          ? Promise.resolve()
          : aiFetch({ method: "GET", headers: { accept: "application/json" } }, 3000)
              .then(function (r) {
                ai.on = r.ok && r.data.configured === true;
                // The demo note promises a trained version later; once the key
                // is live that version is what the visitor is talking to.
                var note = ai.on && panel.querySelector(".gk-chat-note");
                if (note) note.hidden = true;
              }, function () { ai.on = false; });
      }
      return ai.checked;
    }

    // The server's own cap, without cutting a surrogate pair (an emoji) in half.
    function clip(text) {
      return String(text).slice(0, 500).replace(/[\uD800-\uDBFF]$/, "");
    }

    /* One answer at a time, so replies land in the order the questions were
       asked and the model sees the conversation the visitor saw. A failed
       request answers that message from the script and leaves AI mode on;
       only the server saying it has no key turns it off. */
    function respond(question, delay) {
      checkAi().then(function () {
        if (!ai.on) {
          setTimeout(function () { say("bot", reply(question)); }, delay);
          return;
        }
        var next = function () { return askAi(question); };
        ai.queue = (ai.queue || Promise.resolve()).then(next, next);
      });
    }

    function askAi(question) {
      if (!ai.on) { say("bot", reply(question)); return; }
      ai.turns = ai.turns.concat({ role: "user", content: clip(question) }).slice(-10);
      return aiFetch({
        method: "POST",
        headers: { "content-type": "application/json", accept: "application/json" },
        body: JSON.stringify({ messages: ai.turns, conversation: conversationId(), page: location.pathname }),
      }, 15000)
        .then(function (r) {
          if (r.status === 503 && r.data.configured === false) ai.on = false;
          return r.ok && typeof r.data.reply === "string" ? r.data.reply.trim() : "";
        }, function () { return ""; })
        .then(function (text) {
          var answer = text || reply(question);
          ai.turns = ai.turns.concat({ role: "assistant", content: clip(answer) }).slice(-10);
          say("bot", answer);
        });
    }
  ...
  function alignPageForms() {
    ...
    Array.prototype.forEach.call(forms, function (form) {
      ...
      form.addEventListener("submit", function (event) {
        ...
        if (!f.endpoint) {
          say(f.thanks || "Thanks. We will be in touch shortly.");
          form.reset();
          track("generate_lead", { lead_source: "page form" });
          return;
        }
        fetch(f.endpoint, {
          method: "POST",
  ...
  function captureTemplateName() {
    /* Templates name themselves twice and rarely the same way: the navbar
       ...
       as "Acme Plumbing and Heating Plumbing". Both forms are collected and the
  ...
  var TRADE_WORDS = ("plumbing plumbers roofing dental dentist spa med medical wellness centre " +
    ...
    /* Templates named after an adjective rather than a coined word: Luxury
       ...
       becoming "Acme Plumbing service"), so those keep the full-name swap only. */
  ...
  function renameTemplate() {
    ...
    /* Elements first. A footer brand is often marked up as "FlowRight" in one
       ...
       gave "Acme Plumbing and Heating Plumbing". Anything whose whole text is
  ...
  /* --------------------------------------------------------------- analytics
     Google Analytics 4, only when SITE.ga4 holds the site's measurement ID,
     set in demo-site.js beside the rest of the business's details:
       "ga4": "G-XXXXXXXXXX",
     Without it nothing loads and track() does nothing, so a site that never
     sets it behaves exactly as before. A ?site= preview builds its own SITE
     without one, so a prospect's preview is never counted.

     It is Google's standard tag, which counts the page views itself, plus
     three events a local business cares about: click_to_call (any phone
     link), generate_lead (the quote form or the page's own form reaching its
     thank you) and chat_start (the visitor's first message in the chat).
     There is no cookie notice here, and a UK or EU site needs one before
     the ID goes in. */
  var GA4 = "";
  ...
  function startAnalytics() {
    var id = typeof SITE.ga4 === "string" ? SITE.ga4.trim() : "";
    if (!/^G-[A-Z0-9]{4,20}$/.test(id)) return;
    GA4 = id;

    window.dataLayer = window.dataLayer || [];
    // The tag reads each call's arguments object, exactly as Google's snippet pushes it.
    ...
    /* One listener for every phone link on the page, the chat's call row
       included. Capturing, so a template's own click handler cannot stop the
       tap reaching it. */
    document.addEventListener("click", function (event) {
      var link = event.target && event.target.closest ? event.target.closest('a[href^="tel:"]') : null;
      if (link) track("click_to_call");
    }, true);
  }

  function start() {
    // Before the rest, so a page view is counted even if a later step fails.
    startAnalytics();
  ...
  var STORE = "https://YOUR-PROJECT.supabase.co";
