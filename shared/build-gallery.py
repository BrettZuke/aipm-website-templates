...
def card(it):
    ...
    if live:
        # A compiled app that cannot run from a subfolder carries an "href" in
        # templates.json; its files still live here so it can be lifted and deployed.
        href = it.get('href') or f'templates/{slug}/'
        ext = ' target="_blank" rel="noopener"' if href.startswith('http') else ''
        return (f'      <a class="tpl" href="{href}"{ext}>\n'
